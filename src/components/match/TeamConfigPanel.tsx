...
export function TeamConfigPanel() {
  ...
  return (
    <div className="relative rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5 space-y-6">
      {!canManageMatch && <div className="rounded-xl border border-violet-400/30 bg-violet-400/10 px-4 py-3 text-xs font-bold text-violet-100">Salt Okunur — Bu toplulukta maç ayarlarını yalnızca Kurucu Admin, Admin ve Moderatör değiştirebilir.</div>}
      <fieldset disabled={!canManageMatch} className={`contents ${!canManageMatch ? "[&_button]:cursor-not-allowed [&_button]:opacity-55 [&_input]:cursor-not-allowed [&_input]:opacity-55" : ""}`}>
      {/* BAŞLIK */}
